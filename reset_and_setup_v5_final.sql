-- MASTER RESET & SETUP V5 (Final Fix)
-- -----------------------------------------------------------------------------
-- This script resets schema AND fixes the "Auth User exists but Profile missing" bug.
-- -----------------------------------------------------------------------------

-- 1. CLEANUP (Drop public tables)
...
begin
  ...
  on conflict (id) do nothing; -- Prevent errors if manually inserted
  return new;
end;
...
-- 7. CRITICAL FIX: Backfill Profiles for Orphaned Auth Users
-- This fixes the "User Exists / Profile Missing" bug.
do $$
begin
