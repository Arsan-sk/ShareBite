-- MASTER RESET & SETUP V6 (Corrected Schema)
-- -----------------------------------------------------------------------------
-- This script fixes the "Listing not creating" bug by restoring missing columns.
-- -----------------------------------------------------------------------------
...
create table public.profiles (
  ...
  license_number text, -- Added back
  phone_number text,   -- Added back
  location text,       -- Added back
  latitude float,      -- Added back
  longitude float,     -- Added back
...
create table public.listings (
  ...
  pickup_window_start timestamp with time zone, -- Added back
  pickup_window_end timestamp with time zone,   -- VISIBILITY FIX (Used in Action)
...
create table public.pickups (
  ...
  notes text, -- Added back
  pickup_time timestamp with time zone, -- Added back
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
...
-- 7. BACKFILL PROFILES (Critical for Auth Sync)
do $$
begin
  ...
  select 
    ...
    email, 
    split_part(email, '@', 1), 
    coalesce((raw_user_meta_data->>'role')::user_role, 'resident')
  from auth.users
  on conflict (id) do nothing;
end;
$$;
